...
# RUTAS PARA COPIAS 'cp_'
@app.route('/cp/pollo')
def cp_pollo():
    if not loggedin():
        flash('Debes iniciar sesión para ver el menú de Pollos.', 'error')
        return redirect(url_for('inicio_secion'))
    return render_template('cp_pollo.html', loggedin=True, nombre=session.get('nombre'))
...
def cp_pizza():
    if not loggedin():
        flash('Debes iniciar sesión para ver el menú de Pizzas.', 'error')
        return redirect(url_for('inicio_secion'))
    return render_template('cp_pizza.html', loggedin=True, nombre=session.get('nombre'))
        ...
        with conn.cursor() as cursor:
            ...
            for row in rows:
                producto = {
                    ...
                    # Imagen por defecto (no existe en BD)
...
def procesar_pago():
    ...
    metodo = request.form.get('idMetodoPago')
    usuario_id = session.get('id') or session.get('id_usuario')

    if not usuario_id:
        flash("Debes iniciar sesión para realizar el pago.", "error")
        return redirect(url_for('inicio_secion'))
    ...
    try:
        with conn.cursor() as cursor:
            ...
            print(f"🧾 Pedido creado (id={pedido_id}) para usuario {usuario_id}")
        ...
        conn.rollback()
        print("❌ Error en procesar_pago:", type(e).__name__, e)
...
# ----------------------------------------------------------------------
# 📚 FUNCIONES DE BASE DE DATOS PARA COMENTARIOS Y RECLAMACIONES
# ----------------------------------------------------------------------

def obtener_comentarios_db():
    """Recupera todos los comentarios ordenados por fecha."""
    conn = get_db_connection()
    if not conn: return []
    try:
        with conn.cursor() as cursor:
            query = """
                SELECT usuario_nombre, texto, 
...
def guardar_comentario_db(usuario_nombre, texto):
    """Guarda un nuevo comentario."""
    conn = get_db_connection()
    if not conn: return False
    try:
        with conn.cursor() as cursor:
            query = "INSERT INTO Comentarios (usuario_nombre, texto) VALUES (%s, %s)"
            cursor.execute(query, (usuario_nombre, texto))
            conn.commit()
            return True
    except Exception as e:
...
def obtener_reclamos_usuario_db(usuario_id):
    """Recupera SÓLO los reclamos de un usuario específico."""
    conn = get_db_connection()
    if not conn: return []
    try:
        with conn.cursor() as cursor:
...
def guardar_reclamo_db(usuario_id, texto):
    """Guarda un nuevo reclamo asociado al ID del usuario."""
    conn = get_db_connection()
    if not conn: return False
    try:
        with conn.cursor() as cursor:
            query = "INSERT INTO Reclamaciones (usuario_id, texto) VALUES (%s, %s)"
...
# ----------------------------------------------------------------------
# 🌐 RUTAS DE COMENTARIOS Y RECLAMACIONES
# ----------------------------------------------------------------------

# Rutas para el Libro de Comentarios
@app.route("/comentarios")
def comentarios():
    # Recupera todos los comentarios para mostrarlos (Ahora la función está definida)
    comentarios_data = obtener_comentarios_db() 
...
def guardar_comentario():
    ...
    usuario = session.get("apodo") or session.get("nombre") 
    
    if guardar_comentario_db(usuario, texto):
        flash('Comentario enviado exitosamente.', 'success')
    else:
        flash('Hubo un error al guardar tu comentario.', 'error')
        
    return redirect(url_for("comentarios"))


# Rutas para el Libro de Reclamaciones
@app.route("/libro_reclamaciones")
def libro_reclamaciones():
    if not session.get("loggedin"):
        flash('Debes iniciar sesión para acceder al libro de reclamaciones.', 'error')
        return redirect(url_for("inicio_secion"))

    usuario_id = session.get("id") 
    reclamos_data = obtener_reclamos_usuario_db(usuario_id) # (Ahora la función está definida)

    return render_template(
        "L-reclamaciones.html",
        loggedin=session.get("loggedin"),
        nombre=session.get("nombre"),
...
def guardar_reclamo():
    if not session.get("loggedin"):
        flash('Debes iniciar sesión para enviar un reclamo.', 'error')
        return redirect(url_for("inicio_secion"))

    texto = request.form.get("reclamo")
    usuario_id = session.get("id") 

    if guardar_reclamo_db(usuario_id, texto):
        flash('Reclamo enviado y registrado con éxito.', 'success')
    else:
        flash('Hubo un error al guardar tu reclamo.', 'error')
