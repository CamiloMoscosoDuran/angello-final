    ...
    try:
        with conn.cursor() as cursor:
            ...
            if not id_producto and nombre:
                cursor.execute("SELECT idProducto, precio FROM Producto WHERE nombre = %s LIMIT 1", (nombre,))
            ...
            if exist:
                ...
                if not precio or precio == 0:
                    cursor.execute("SELECT precio FROM Producto WHERE idProducto=%s LIMIT 1", (id_producto,))
                    p = cursor.fetchone()
                    precio = float(p['precio']) if p else precio
