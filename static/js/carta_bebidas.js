document.addEventListener('DOMContentLoaded', function () {
  ...
  function showSection(targetId) {
    ...
    if (target) {
      ...
      // mantener visible el nav de secciones para permitir navegación entre secciones
      const nav = document.getElementById('sections-nav');
      if (nav) {
        nav.style.display = 'flex';
        // Hacer la navegación más compacta cuando está en modo filtrado
  ...
  function showAll() {
    ...
    // restaurar nav de secciones con padding original
    const nav = document.getElementById('sections-nav');
    if (nav) {
      nav.style.display = 'flex';
      nav.style.padding = '2rem'; // Restaurar padding original
    }
    showAllBtn.style.display = 'none';
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }
  ...
  // Agregar event listeners para todos los botones de "Añadir al carrito"
  document.querySelectorAll('.add-to-cart').forEach(function(button) {
    button.addEventListener('click', function(e) {
      ...
      // No generar ID, enviar null para que el backend busque por nombre
      
      // Llamar a la función de agregar al carrito (debe estar definida en carrito.js)
      if (typeof agregarAlCarrito === 'function') {
        agregarAlCarrito(null, nombre, parseFloat(precio), img);
      } else {
        console.error('La función agregarAlCarrito no está disponible');
        // Fallback: mostrar alerta simple
        alert('Producto agregado: ' + nombre + ' - S/ ' + precio);
      }
    });
  });
});
